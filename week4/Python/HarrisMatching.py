...
for i1 in range(npts1):
    for i2 in range(npts2):
        f_mean = np.mean(patches1[:,:,i1])
        g_mean = np.mean(patches2[:,:,i2])
        f_norm = patches1[:,:,i1] - f_mean
        g_norm = patches2[:,:,i2] - g_mean
        numerator = np.sum(f_norm*g_norm)
        denominator = np.sqrt(np.sum(f_norm**2)*np.sum(g_norm**2))
        nccmat[i1, i2] = numerator / (denominator + 1e-8)

# Get max similarity between image patches
ss1 = np.amax(nccmat, axis=1)
ids1 = np.argmax(nccmat, axis=1)
ss2 = np.amax(nccmat, axis=0)
ids2 = np.argmax(nccmat, axis=0)

# Pairing most matched patches
pairs = []
for k in range(npts1):
    if k == ids2[ids1[k]]:
        pairs.append(np.array([k, ids1[k], ss1[k]]))
...
# Sort NCC similarity on descending manner
