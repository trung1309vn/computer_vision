...
# np.outer(np.sqrt(sig[0])*v[0,:], np.sqrt(sig[0])*u[:,0]) = g
u, sig, v = np.linalg.svd(g)
g_hor = np.sqrt(sig[0])*v[0,:]
g_ver = np.sqrt(sig[0])*u[:,0]

gflt_imns = conv1(conv1(imns, g_hor, axis=0, mode='reflect'), g_ver, axis=1, mode='reflect')
gflt_imng = conv1(conv1(imng, g_hor, axis=0, mode='reflect'), g_ver, axis=1, mode='reflect')
...
def median_filter(img, wsize):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ##--your-code-starts-here--##
            output[i,j] = np.median(img[iMin:iMax,jMin:jMax])
            ##--your-code-ends-here--##
